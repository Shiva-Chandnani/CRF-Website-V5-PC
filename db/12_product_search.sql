...
-- RPC over v_products: tsvector prefix matching, with a pg_trgm fuzzy fallback
-- that only kicks in when the prefix match finds nothing (so typo-tolerance
-- never dilutes a clean prefix/exact match with unrelated rows).
  ...
  scored as (
    select f.*,
           lower(concat_ws(' ',
             ...
             array_to_string(f.color, ' '), f.item_occasion)) as doc
    from filtered f
  ),
  normed as (
