...
| 14 | Privacy page + security baseline | 1 (draft + CSP baseline) + 3 (CSP hardening + RLS audit) | 🔜 **NEXT UP** — Phase 1 half done (privacy.html + CSP baseline shipped WT-4). Remaining: CSP hardening (frame-ancestors→HTTP header, tighten unsafe-inline) + RLS audit. See the "🔜 NEXT UP — #14" subsection in §7. **Last remaining Phase 3 feature.** |
...
### 🔜 NEXT UP — #14 CSP/RLS hardening (Phase 3 close)

The **only** remaining Phase 3 feature. Run it through the standard agentic cycle (brainstorm → spec → plan → worktree → subagent-driven execute → verify → review → finish). Three workstreams:

1. **Clickjacking / `frame-ancestors`** — currently NOT enforced: it was dropped from the `<meta>` CSP in Phase 1 WT-4 because browsers ignore `frame-ancestors` delivered via `<meta http-equiv>` (see §7 WT-4 notes + top-banner reminder #3). Move it to an **HTTP response header** (`Content-Security-Policy: frame-ancestors 'none'` + `X-Frame-Options: DENY`). For local dev, add the headers in `serve.mjs`; for production, document the host/CDN header config (this is a static site — the real enforcement is at the host).
2. **Tighten `unsafe-inline`** — the current CSP allows `'unsafe-inline'` for `script-src` and `style-src`. Audit inline `<script>`/`<style>` usage across the 14 pages and move toward nonces/hashes (or at minimum document why inline is retained). Note the site is no-build with lots of inline page styles + inline module scripts, so full removal is a real effort — scope it during brainstorming (may land as "style-src tightened, script-src documented" rather than全removal).
3. **RLS audit** — systematically re-verify Row-Level Security across every user-data table: `profiles`, the 4 `customer_*` measurement tables + 4 `v_latest_*` views (security_invoker), `carts`, `orders`, `payments`, `newsletter_subscribers`. Confirm owner-only isolation, no view leaks (the WT-3 `security_invoker` bug is the cautionary tale), and that Edge-Function-only write tables (orders/payments) have no client write policy. Existing tests to lean on / extend: `test-profile-rls`, `test-measurements-rls`, `test-cart-rls`, `test-orders-rls`. Consider a single consolidated `test-rls-audit.mjs`.

Backlog #14 spans Phase 1 (privacy page + CSP baseline — done) + Phase 3 (this hardening). The privacy page (`privacy.html`) already shipped in WT-4.
