...
> **Last session ended** at: **✅ SITE-REVIEW FIXES SHIPPED — "Pants" → "Trousers" relabel + hidden "pants" search alias** (2026-07-23, interactive site-review batch). British house terminology: all **user-facing display names** now read "Trousers" while every id/slug/URL stays `pants`/`dress-pants`/`linen-pants`/`pants-*` (nothing to re-point). **DB:** `db/17_rename_pants_to_trousers.sql` renames `.name` display strings only on `categories`/`item_types`/`subcategories`/10 pants-group `customization_categories` (idempotent, txn-wrapped, applied live + verified). **JS:** `shop-page.js` (header title + `SUBCATEGORY_LABEL`), `mega-menu.js` (labels), `product-page.js` (`CATEGORY_LABEL` breadcrumb). **Search alias:** `db/18_search_pants_alias.sql` re-creates `search_products` with one change — rows whose stable `category_id = 'pants'` get the token **"pants"** appended to their search document **only** (never any displayed field), so both "pants" and "trousers" resolve trouser products at parity (35=35, verified 0 displayed names contain "pant"). **Tests:** `test-product-search.mjs` gains a "pants" alias-parity assertion; product-search (9) + shop-search (7) + search-overlay (9) all green. **Git:** merged no-ff to `main` and pushed — `main` == `origin/main`; the stale `chore/site-review-fixes` branch is merged + deleted. ⚠️ Pre-launch reminders (unchanged): re-enable Supabase email confirmation + SMTP; activate Stripe account + LIVE webhook; order-confirmation email; regenerate `sitemap.xml` when the catalogue changes; choose a production host and apply `docs/security/production-headers.md`. *(Open thought: "pants" is currently a hidden search alias only — the visible taxonomy is Trousers everywhere.)*
>
> **Prior — ✅ PHASE 5 (sub-project A) — INTERNAL STAFF CRM SHIPPED on branch `phase-5/staff-crm`** (2026-07-15, backlog #9 + the "customers" half of #8). A staff-only website CRM built as a **spoke** of the future central retail POS (see the "CRM/POS strategy" note + the spec's "Future POS connector alignment" section). **Access model:** additive staff-read RLS — `is_staff()` (SECURITY DEFINER, pinned `search_path`) + staff `SELECT` policies added *alongside* the owner-only ones (Postgres OR-combines SELECT policies, so customers stay fully isolated — verified: `test-rls-audit.mjs` still green). **DB (`db/14_staff_crm.sql` + `db/15_crm_metrics.sql` + `db/16_lock_profile_role.sql`):** staff-read policies on profiles/orders/payments/4 measurement tables; **role lock** — a `before update` trigger (`guard_profile_role`) blocks any authenticated API caller from changing `profiles.role` (role is provisioned only out-of-band via service_role / `run-sql`, where `auth.uid()` is null), closing a privilege-escalation hole where a customer could self-promote to `staff` and read the whole customer base (found + fixed in final code review; regression-tested in `test-admin-rls.mjs`); POS seam columns on `profiles` (`pos_customer_id` unique = immutable POS `customers.id` UUID, `source` in website|pos|manual|import, `last_synced_at`); `customer_notes` + `customer_tags` (staff-only RLS, insert requires `author_id = auth.uid()`); staff-only `crm_metrics()` RPC (aggregate tiles + 12-month series, raises for non-staff). **Frontend:** `requireStaff()` gate in `js/auth.js`; `js/crm.js` data layer; two `requireStaff`-gated `noindex` pages — `admin-customers.html` (metrics strip + trend chart + searchable customer table) + `admin-customer.html` (contact/orders/payments/measurements/notes/tags), house-styled via frontend-design + dataviz. **Tests:** `test-admin-rls.mjs` (staff-read + isolation + anon lockout), `test-crm-metrics.mjs`, `test-admin-pages.mjs` (puppeteer e2e — customer bounced, staff CRUD round-trip); 16-page CSP sweep + `test-rls-audit` + token-discipline + layout-mount all green; both pages visually verified (1440 + 375). **Deferred:** the POS connector itself (not built — only the seam), full analytics dashboard (#8), and **sub-project B** (customer-facing account enhancements). ⚠️ Pre-launch reminders (unchanged): re-enable Supabase email confirmation + SMTP; activate Stripe account + LIVE webhook; order-confirmation email; regenerate `sitemap.xml` when the catalogue changes; choose a production host and apply `docs/security/production-headers.md`. **Git:** to be merged no-ff to `main` on completion (branch `phase-5/staff-crm`, off `435d704`).
>
> **Prior — ✅ PHASE 4 COMPLETE — CUSTOMIZATION EXPANSION SHIPPED on branch `phase-4/customization-expansion`** (2026-07-15, backlog #1 extend). The "Customize" drawer now runs for standalone **`formal-jacket`** and **`dress-pants`** in addition to the Suit, reusing the existing catalogue (no new categories/options/SVGs, no price deltas). Data-driven: (A) migration `db/13_customization_jacket_pants.sql` seeds the junction rows (11 jacket → `formal-jacket`, 10 pants → `dress-pants`; idempotent `on conflict do nothing`). (B) a single `CUSTOMIZABLE` map in `js/product-page.js` drives button gating + the per-garment label ("Customize Your Suit/Jacket/Trousers") and passes `garment_noun` into the drawer. (C) `js/customizer.js` renders the title from `garment_noun` and shows category-group section headers **only when >1 group is present** — Suit keeps its "Jacket"/"Trouser" headers, single-cut garments render flat. (D) **Bug fix:** `js/cart-page.js` now merges the customization catalog index across **every** item type in the cart (was: only the first line's), so a mixed cart no longer drops a later line's spec rows. Tests: new `scripts/test-customization-item-types.mjs` (catalog assertions: jacket 11 / pants 10, group-pure, one default each) + `scripts/test-customizer-flow.mjs` extended with assertions for Suit + Jacket + Trouser drawers + mixed-cart spec (10 Phase-4 checks, all green). Full regression (token-discipline, layout-mount, 14-page CSP sweep) green; drawer screenshots verified (Jacket/Trouser flat, Suit unchanged). ⚠️ Pre-launch reminders (unchanged): re-enable Supabase email confirmation + SMTP; activate Stripe account + LIVE webhook; order-confirmation email; regenerate `sitemap.xml` when the catalogue changes; choose a production host and apply `docs/security/production-headers.md`. **Git:** to be merged no-ff to `main` on completion (branch `phase-4/customization-expansion`, off `53d2b6f`).
>
...
| `/admin-customers.html` `?q=<query>` | [admin-customers.html](admin-customers.html) | **Staff CRM home** (Phase 5) — `requireStaff` gated + `noindex`. Metrics strip (via `crm_metrics()` RPC) + by-month trend chart + searchable/paginated customer table (`js/crm.js` → `listCustomers`). Rows link to the detail page |
| `/admin-customer.html?id=<uuid>` | [admin-customer.html](admin-customer.html) | **Staff customer detail** (Phase 5) — `requireStaff`; contact + POS-seam fields, orders + payments, latest measurements, and the only write surfaces: staff notes + tags. Data via `js/crm.js` `getCustomer`/`addNote`/`addTag`/`removeTag` |

**Site-wide search (Phase 3):** every page's shared header carries a search icon (`[data-search-btn]`) wired by `js/search-overlay.js` to a typeahead overlay — live ranked product results (thumbnail + name + "from THB …") linking to PDPs, plus a "See all results →" / Enter handoff to `/shop.html?q=…`. Not a page of its own; it mounts on the shared header on all 14 pages.
...
STAFF CRM (Phase 5; db/14_staff_crm.sql + db/15_crm_metrics.sql):
is_staff() — SECURITY DEFINER boolean (caller's profiles.role in staff|admin; search_path=public; no RLS recursion)
profiles seam columns: pos_customer_id text UNIQUE [= POS customers.id UUID, immutable external id], source text [website|pos|manual|import], last_synced_at timestamptz
staff-read RLS: ADDITIVE "*_staff_select" SELECT policies using is_staff() on profiles/orders/payments/4 customer_* tables (owner-only policies UNTOUCHED; SELECT policies OR-combine → customers stay isolated)
customer_notes (id, customer_id → profiles cascade, author_id → profiles set null, body, source, created_at) — staff SELECT + staff INSERT (author_id = auth.uid()); no customer/anon access
customer_tags (customer_id → profiles cascade, tag [1..40 chars], author_id, created_at, pk (customer_id, tag)) — staff SELECT/INSERT/DELETE
crm_metrics() — staff-only SECURITY DEFINER RPC returning json {total_customers, new_this_month, paid_orders, revenue_thb, aov_thb, by_month[]}; raises 'forbidden' for non-staff
```
...
| Table | Rows |
|---|---|
| `categories` | 7 (Suits, Shirts, **Trousers** [id `pants`], Coats, Jackets & Blazers, Mandarin Collar, Accessories). Display renamed Pants→Trousers 2026-07-23 (`db/17`); id/slug unchanged. |
| `subcategories` | 22 |
...
│   ├── search-overlay.js         # Phase 3 — header typeahead overlay: mounts on crf:layout-ready, debounced quickSearch, accessible combobox (aria-activedescendant, focus trap, Esc), XSS-safe render, PDP links + See-all → shop.html?q=
...
│   ├── 14_staff_crm.sql                 # Phase 5 — staff-read RLS + is_staff() + POS-seam cols + customer_notes/tags
│   ├── 15_crm_metrics.sql               # Phase 5 — staff-only crm_metrics() RPC (tiles + 12-month series)
│   ├── 16_lock_profile_role.sql         # Phase 5 — guard_profile_role trigger: blocks API callers from changing profiles.role
│   ├── 17_rename_pants_to_trousers.sql  # Site-review — rename display .name Pants→Trousers on categories/item_types/subcategories/pants-group customization_categories; ids/slugs untouched; idempotent + txn-wrapped
│   ├── 18_search_pants_alias.sql        # Site-review — re-creates search_products so category_id='pants' rows get a hidden "pants" token in the search doc only; "pants" & "trousers" both resolve trousers
...
| 8 | Admin dashboard (analytics + customers) | 5 | 🟡 partial — "customers" half done (staff CRM, 2026-07-15); full analytics dashboard deferred |
| 9 | CRM sync | 5 | 🟡 V1 website CRM done (staff CRM sub-project A, 2026-07-15, branch `phase-5/staff-crm`) w/ POS seam; POS connector deferred (POS not yet connectable); customer-facing sub-project B deferred |
...
| 14 | Privacy page + security baseline | 1 (draft + CSP baseline) + 3 (CSP hardening + RLS audit) | ✅ done — Phase 1 half (privacy.html + CSP baseline, WT-4) + Phase 3 half (CSP/RLS hardening, shipped 2026-07-14, branch `phase-3/csp-rls-hardening`): HTTP security headers, `script-src 'self'` via inline-script externalization, consolidated `test-rls-audit.mjs`. See the "Phase 3 — CSP/RLS hardening (SHIPPED)" subsection in §7. |
...
| 5 — Operations | Admin + CRM | 8, 9 | 🟡 In progress. Sub-project A (internal staff CRM) shipped 2026-07-15 (branch `phase-5/staff-crm`): staff-read RLS, POS seam, notes/tags, metrics, 2 admin pages. Deferred: POS connector (POS not ready), full analytics dashboard, customer-facing sub-project B. |
...
### Phase 3 — product search (SHIPPED 2026-07-10) — backlog #3 + #4

Ranked, typo-tolerant product search across the site. Two surfaces, one backend. Built subagent-driven from `docs/superpowers/plans/2026-07-10-phase-3-product-search.md`; spec `docs/superpowers/specs/2026-07-10-phase-3-product-search-design.md`.

- **DB** `db/12_product_search.sql` — idempotent (`create extension if not exists pg_trgm` + `create or replace function`), applied live via `scripts/run-sql.mjs`. `search_products(search_query text, p_category_id, p_subcategory_id, p_fabric_type_id, p_pattern, p_color)` `returns setof v_products`, `stable`, `security invoker`, `grant execute to anon, authenticated`. Searchable text is a normalized concat of `display_name, item_type_name, fabric_brand, fabric_family, fabric_type_name, design_name, fabric_number, pattern, color[], item_occasion`. **Two-tier match:** tsvector `'simple'` **prefix** search (`to_tsquery(...:* )`) first; a `pg_trgm` `word_similarity` fuzzy fallback runs ONLY when the prefix pass returns zero rows (blending fuzzy additively polluted short queries like "sui"/"WL-1102" with trigram noise — the tiered design keeps exact/prefix precise while still catching typos like "wollen"→wool). Structured filter params AND-narrow; blank query = filter-only ordered by `display_name`. `pattern` is compared via `::text` cast (it's an enum). No new RLS surface (v_products already grants anon SELECT).
- **JS** `js/data-loader.js` — added `productSearch(query, filters)` (RPC when query present, else the existing filter-only `searchProducts`) and `quickSearch(query, limit=6)` (query-only, capped, for the overlay).
- **Header overlay** `js/search-overlay.js` + styles appended to `css/base.css` — mounts on `crf:layout-ready`, wires the shared header `[data-search-btn]` on **all 14 pages**. Accessible **combobox**: `role=combobox` + managed `aria-activedescendant`, ArrowUp/Down navigate results (wrap-around, `is-active` highlight), Enter follows the active option (else → `shop.html?q=`), focus trap, `Esc` closes + restores focus, `#search-overlay[hidden]{display:none}` keeps the closed overlay out of the tab order. **XSS-safe:** the query is never interpolated into `innerHTML` (empty/error states built via `textContent`; catalogue strings escaped via an `esc()` helper). Debounced 200ms with a `seq` stale-response guard. Result rows: thumbnail (`fabricImageUrl`) + `display_name` + "from THB …".
- **Shop** `shop.html` — reads `?q=` on load (prefills `#searchInput`), routes `loadProducts()` through `productSearch(state.query, filters)` (combined AND with the sidebar), dropped the old client-side substring filter, debounced 250ms re-query. **Stale-response generation guard** (`loadSeq`) on `loadProducts` + the pending search debounce is cancelled on filter/sort change (prevents a slow text-search response from overwriting a newer filter-scoped result). `sortProducts` default/'recommended' branch made a no-op so server relevance order survives.
- **Tests (all green):** `test-product-search.mjs` (node/RPC, 8 cases: prefix, fuzzy, fabric-number, combined query+filter, blank, ranking, punctuation-only), `test-search-overlay.mjs` (puppeteer, 9 cases incl. closed-overlay `display:none`, **XSS payload does not execute**, ArrowDown activates option via aria-activedescendant), `test-shop-search.mjs` (puppeteer: `?q=` prefill, narrows, clear restores). CSP sweep **extended to open the overlay + type on every page** — all 14 clean. Core regression (layout-mount, token-discipline, swatch-prefers-hero, customizer-flow, design-hero-rail, header-account-swap, newsletter-submit) green.
- **Deferred follow-ups (non-blocking):**
  1. **Scalability:** `search_products` seq-scans `v_products` per call — instant at ~10² rows. When the catalogue grows large enough to show latency, migrate to a **materialized search table with a generated `tsvector` column + GIN trigram index**, kept in sync via triggers on the base catalogue tables. The JS/overlay/shop surfaces stay unchanged — only the RPC internals swap.
  2. **Test coverage is data-limited:** the catalogue is currently all-wool (6 grouped products), so the shop-search e2e's "narrows" assertions are weakly exercised at the UI layer (the RPC-level test asserts combined query+filter AND with real data). Strengthen once cross-fabric-family SKUs (e.g. linen) are seeded.
  3. **Facet-collapse UX:** typing a query now collapses the sidebar facet lists to only the fabrics/patterns/colors present in the hits (consistent with how structured filters already behaved — combined-AND by design). A very loose query can shrink the sidebar to 1–2 options; worth a design sign-off / "matched" labelling pass in a later polish.

### Phase 3 — SEO baseline (SHIPPED 2026-07-13) — backlog #11

Production-ready SEO baseline, **hybrid approach**: static pages hardcode meta in raw HTML (robust for crawlers + non-JS social scrapers); dynamic pages inject meta via JS after catalogue data loads. Built subagent-driven from `docs/superpowers/plans/2026-07-13-seo-baseline.md`; spec `docs/superpowers/specs/2026-07-13-seo-baseline-design.md`. Merged no-ff (`606b237`), branch deleted, pushed to origin.

- **`js/meta.js`** — real `setMeta({title, description, canonical, ogImage, ogType, jsonLd, robots})`. Upserts `<title>`, meta description, canonical, Open Graph, Twitter, and injects one `<script type="application/ld+json" data-meta-jsonld>` per `jsonLd` entry (accepts object or array). **Adopts existing hardcoded tags in place** — the upsert matches a tag by `name`/`property`/`rel` (NOT requiring the `data-meta` marker), updates it, and stamps `data-meta`. So a page that both hardcodes a tag AND calls `setMeta` ends up with exactly one (verified: shop base has 1 description tag). Exports `SITE_ORIGIN` = `https://countryroadfashions.com`, `SITE_NAME`, `DEFAULT_OG_IMAGE` (a Supabase render-endpoint suit hero, ~1200px), `canonicalFor(pathAndQuery)`.
- **Static indexable pages** (index, shop, book-appointment, in-store, privacy) — hardcoded `<head>`: title + meta description + `link rel=canonical` + OG + Twitter. `index.html` also emits `ClothingStore` (LocalBusiness; real Bangkok PostalAddress + founder Tourmal Chandnani + foundingDate 1951 + email + priceRange ฿฿฿) **and** `WebSite` + `SearchAction` (→ `/shop.html?q={search_term_string}`) JSON-LD. `in-store.html` emits `ClothingStore`.
- **Dynamic pages** — `product.html` `applyPdpMeta()` (called in `init()` after `renderAccordion`, before `crf:pdp-ready`) emits `Product` + `BreadcrumbList` JSON-LD and sets a **design-stripped canonical** (`/product.html?item=X&fabric=Y`, no `design=`) → all 35 design variants consolidate onto **6 canonical item×fabric PDPs**. `Product.sku` = composite `item__fabric` (deliberate deviation from spec's `fabric_number`, which would collide across item types); `Offer` price = item×fabric THB, `InStock`. PDP head also carries a **generic static fallback** description/og:image for non-JS scrapers (setMeta overwrites in place when JS runs). `shop.html` `applyShopMeta()` (called in `init()`, `refresh()`, and the debounced search handler) sets a search-/category-aware title + base-consolidating canonical on `?q=`, and a `BreadcrumbList` (adds the category as position 3 when a category filter is active).
- **`robots.txt`** (new, root) — `Allow: /`, `Disallow` on the 8 private pages, `Sitemap:` line. `serve.mjs` gained `.txt`/`.xml` MIME types. The same **8 private pages carry `<meta name="robots" content="noindex, nofollow">`** (login, signup, forgot-password, reset-password, account, measurements, cart, order-confirmation).
- **`sitemap.xml`** (committed, GENERATED by `scripts/generate-sitemap.mjs`) — reads `v_products` via the anon key, emits 5 static pages + 6 canonical item×fabric PDP URLs (design-stripped, `&`→`&amp;`). **⚠️ Regenerate + recommit whenever the catalogue changes** (`node scripts/generate-sitemap.mjs`). Footer `components/footer.html` "Sitemap" link repointed `index.html` → `/sitemap.xml` (Accessibility link left as a placeholder — out of scope).
- **Tests (all green):** `scripts/test-seo-meta.mjs` (44 checks: static-page raw-HTML meta/OG/canonical/JSON-LD, 8 private `noindex`, robots.txt + sitemap.xml served + well-formed + no `design=`, PDP setMeta design-stripped canonical + Product/BreadcrumbList + single-managed-tag/no-dupes, shop `?q=` title/canonical). 14-page CSP sweep stays clean (ld+json is a data block, not subject to `script-src`). Core regression (layout-mount, token-discipline, product-search, search-overlay, shop-search) green.
- **Deferred (per user):** deeper/continuous SEO (richer per-page copy, more product URLs in sitemap, a purpose-built 1200×630 OG share card instead of the render-endpoint hero, per-PR meta checks) is **deferred until the full product catalogue is uploaded**. Accepted baseline limitation: individual deep-linked PDP social-card previews are generic (client-rendered SPA; full fix = prerender/SSR).

### Phase 3 — CSP/RLS hardening (SHIPPED 2026-07-14) — backlog #14 / Phase 3 close

The last remaining Phase 3 feature. Built via the standard agentic cycle (brainstorm → spec → plan → worktree → subagent-driven execute → verify → review → finish). Spec `docs/superpowers/specs/2026-07-14-csp-rls-hardening-design.md`; plan `docs/superpowers/plans/2026-07-14-csp-rls-hardening.md`. Three workstreams:

- **(A) HTTP security headers.** `serve.mjs` now sends `Content-Security-Policy: frame-ancestors 'none'`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin` on every response (200 + 404). Clickjacking protection finally enforced (it was a no-op in the `<meta>` CSP — browsers ignore meta-delivered `frame-ancestors`). Because this is a static site with **no chosen host yet**, production enforcement is documented in new **`docs/security/production-headers.md`** — drop-in `_headers` (Netlify/Cloudflare), `vercel.json`, and nginx snippets, each also adding `Strict-Transport-Security` (HTTPS-only) + a minimal `Permissions-Policy`. **⚠️ Pre-launch: pick a host and apply one of those configs** — `serve.mjs` headers are dev-only.
- **(B) `script-src 'self'` (removed `'unsafe-inline'`).** All 12 executable inline scripts were externalized verbatim to files under `js/` (the only edit was rewriting relative `./js/…` imports to root-absolute `/js/…`): `js/index-hero-video.js`, `js/book-appointment-tabs.js` (plain IIFEs); `js/shop-page.js`, `js/product-page.js` (two blocks merged), `js/cart-page.js`; and 6 auth bootstraps `js/{account,login,signup,forgot-password,reset-password,order-confirmation}-page.js`. `application/ld+json` blocks stay inline (data, not subject to `script-src`). `script-src` on all 14 pages is now `'self' https://esm.sh https://assets.calendly.com` (no `unsafe-inline`). **`style-src` deliberately retains `'unsafe-inline'`** — the site has many inline `<style>` blocks and no build step, and style injection is not a script-execution vector; a one-line rationale HTML comment sits above each page's CSP block pointing at the spec.
- **(C) Consolidated RLS audit.** New `scripts/test-rls-audit.mjs` seeds two users and asserts, per user-data table, both SELECT-isolation (the *other* user's rows are invisible — the real proof) AND write-lock, across `profiles`, the 4 `customer_*` measurement tables, the 4 `v_latest_*` views (the `security_invoker` leak guard), `carts`, `orders`, `payments`, and `newsletter_subscribers` (previously untested — anon INSERT-only, no anon SELECT/enumeration). 51 checks, idempotent (unique per-run emails + try/finally teardown that also cleans the `ON DELETE SET NULL` orders/newsletter rows). **No RLS misconfigurations found.** The 4 prior per-table RLS tests stay as-is. *Code-review caught (and we fixed) a subtle test bug:* the write-lock checks were vacuous — a supabase-js v2 write without a chained `.select()` returns `data:null`, so a `(data?.length ?? 0) === 0` assertion is always true (would stay green even if write RLS were dropped). Fixed to use admin readback (cross-owner writes) or `.select('id')` on attacker-owned forge inserts, and proven non-vacuous via a sabotage run that forced a FAIL.
- **Tests.** `scripts/test-csp-compliance.mjs` extended: after the 14-page zero-violation sweep it asserts (a) no page's `script-src` contains `unsafe-inline` (stripping HTML comments first so the rationale comment isn't mismatched) and (b) the server sends the four security headers. Full regression green (17 tests incl. product-search, search-overlay, shop-search, customizer-flow, all 4 legacy RLS tests, seo-meta, layout-mount, token-discipline). 4 externalized-page screenshots (index, shop, PDP, book-appointment) show no visual/functional regression.
- **Deferred (non-blocking):** full `style-src` lockdown (needs a build step or per-page hashing); promoting the whole CSP to an HTTP header; tightening `cors.ts` `Access-Control-Allow-Origin` off `*` (noted in the Stripe section). None block launch.

Backlog #14 spans Phase 1 (privacy page + CSP baseline — WT-4) + Phase 3 (this hardening). Both halves are now done → **Phase 3 is complete; Phase 4 (customization expansion to Jacket/Trouser) is next.**
